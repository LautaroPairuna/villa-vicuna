    ...
    <section className="relative min-h-[calc(100dvh-5rem)] overflow-hidden">
      ...
      <div className="relative flex min-h-[calc(100dvh-5rem)] items-end px-4 pb-14 pt-28 text-white md:px-12 md:pb-18">
        <div className="mx-auto w-full max-w-[1200px]">
          <p className="text-[10px] uppercase tracking-[0.35em] text-white/70">{eyebrow}</p>
          <h1 className="mt-5 max-w-5xl text-4xl uppercase tracking-[0.18em] sm:text-5xl lg:text-[4.2rem] lg:leading-[1.12]">
            {title}
          </h1>
          <p className="mt-6 max-w-3xl text-base leading-8 text-white/82">{description}</p>
        </div>
      </div>
    </section>
  );
